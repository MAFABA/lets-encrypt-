...
def perform(achall, delay=3, assume_failed_after=60):
    ...
    if achall.activation_url is not None:
        try:
            _activate_challenge(achall)
        except errors.NetworkError:
            return False

    return _gen_response(achall, delay, assume_failed_after)

def _activate_challenge(achall):
    try:
...
def _gen_response(achall, delay, assume_failed_after):
...
def _handle_polling(achall, delay, assume_failed_after, chall_description):
    wait = True
    while wait:
